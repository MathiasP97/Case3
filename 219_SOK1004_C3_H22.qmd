...
*\[Besvar Oppgave I her\]* Tallet er en måte å regne ut hvor mye KPI'en øker eller faller hver måned ved bruk av prosentvis endring i KonsumPrisIndeksen. \# Oppgave II: Rydd i data
...
```{r}
# løs oppgave II her
df <- subset(df, select = -c(month, year))
...
df_vekter = df %>%
  filter(statistikkvariabel=="Konsumprisindeks (vekter)") %>%
   filter(dato >="2011-01-01" , dato <= "2019-12-01")
...
df_wide <- df_wide %>%
  group_by(dato) %>%
  summarise(across(c('verdi','Konsumprisindeks (vekter)','sjekk <- sum(verdi)'), mean, na.rm=TRUE))


```

*\[Besvar oppgave IIIb her\]* Vi kan bruke SSB's indeks til å se om svarene stemmer overens. \# Oppgave IV: Beregn kraftprisens bidrag til vekst
...
*\[Besvar oppgave IV her.\]* Vi kan se at i rundt 2011 så faller prisen og den gpr nedøver, før etter i 2016 den begynner å stiger i 2016. Og har ikke stoppet å stige siden da. Dennne stigningen er en direkte årsak av det som har skjedd i Ukraina i 2016 og utviklingen av denne konflikten. \# Oppgave V
...
*\[Besvar oppgave V her.\]* Høye kraftpriser vil etterhvert bidra til konsumprisvekst etter som at det kommer et punkt der produsenten eller staten ikke har noe annet valg en å la befolkningen betale mer for kraften slik at de har mulighet til å fortsette å produsere. Når prisen øker og øker så vil KPI'en også øke.
